...
test('IMS feature renders infrastructure, the client logo and screenshots without a system destination', async () => {
  ...
  try {
    ...
    try {
      ...
      assert.ok(dom.window.document.querySelectorAll('.project-detail-page__description li').length >= 10)
      assert.equal(dom.window.document.querySelectorAll('.project-detail-page__tags').length, 1)
