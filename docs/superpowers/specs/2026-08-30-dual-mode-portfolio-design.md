...
an adapted, Clyde-branded version of the public Reference portfolio experience
(`Reference`). The adaptation follows the reference's public information hierarchy,
...
- The source adaptation is derived from `JustineDevs/Portfolio`, which declares
  ...
  attribution notice, and links the upstream source from the Reference-mode footer.
...
- The current Original layout and its assets remain available and are not
  rewritten to depend on the Reference mode.

## Runtime architecture

`App` owns a `portfolioMode` state (`original` or `reference`) persisted in
...
existing desktop/mobile layout. Switching to Reference preserves the current Reference
route in `portfolio-reference-path` and renders the route at `/`, `/about`,
`/projects`, `/projects/:slug`, `/experience`, or `/collection`.

The Reference router is a small history-api adapter, not a new dependency. It
normalizes paths, handles `popstate`, prevents full reloads for internal links,
and falls back to the Reference home page for unknown paths. The mode is inferred as
Reference for a deep public route on first load, allowing direct links to work.

## Reference experience

The new `src/reference` boundary contains the route shell, header/footer, shared
...
2. Original mode renders the current desktop and mobile layouts with no source
   dependency on Reference-only components.
3. Reference mode's six public route families render, navigate internally, and
