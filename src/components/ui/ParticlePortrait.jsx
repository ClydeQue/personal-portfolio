...
function ParticlePortrait({ alt = 'Kenneth Clyde Que', className = '' }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let y = 18; y < bounds.height; y += 37) for (let x = 18; x < bounds.width; x += 37) {
      ...
      for (let y = 0; y < sampleHeight; y += 1) for (let x = 0; x < sampleWidth; x += 1) {
        ...
        if (brightness < 16 || random > Math.min(.97, (brightness + 76) / 220)) continue
        ...
        const jitterX = (jitterRandom - .5) * 1.5
        const jitterY = (random - .5) * 1.5
        context.fillStyle = `rgba(${120 + Math.round(brightness * .5)}, ${170 + Math.round(brightness * .32)}, 255, ${alpha})`
  ...
  return <div className={`particle-portrait ${isReady ? 'is-ready' : ''} ${className}`}>
