    ...
    return (
      <article className="project-card project-card--index">
        <button className="project-card__media" type="button" onClick={() => navigate(detailPath)} aria-label={`View ${project.title}`}>
          ...
          <span className="project-card__index-copy"><strong>{project.title}</strong><em>{project.summary}</em>{project.notice && <span className="project-notice project-notice--compact"><i aria-hidden="true" />Under maintenance</span>}<b>{project.technologies.slice(0, 3).join(' · ')}</b></span>
  ...
  return (
    <article className={`project-card project-card--${variant}`}>
      ...
      {variant !== 'home' && <div className="project-card__body">
        ...
        {project.notice && <p className="project-notice project-notice--compact"><i aria-hidden="true" />Under maintenance</p>}
